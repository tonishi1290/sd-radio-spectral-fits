...
def _row_axes_for_plot(meta: dict, row: Optional[pd.Series], nchan: int,
    ...
    """Return (vel_disp_kms, freq_ghz_or_None, chan_axis) for one row."""
    eff_meta = _row_meta_for_axis(row, meta)
    row_rest_hz_meta = _row_rest_hz_meta(row, meta)
    v_corr = _row_vcorr_kms(row, meta)
    vel_native = vlsrk_axis_for_spectrum(eff_meta, v_corr_kms=float(v_corr), nchan=nchan)
    vel_disp = vel_native
    if (
        row_rest_hz_meta is not None and row_rest_hz_meta > 0 and
        rest_hz_user is not None and rest_hz_user > 0 and
        abs(row_rest_hz_meta - rest_hz_user) > 1.0
    ):
        vel_disp = recalculate_velocity_axis(vel_native, row_rest_hz_meta, rest_hz_user)
...
def _interp_to_velocity_grid(v_src: np.ndarray, y_src: np.ndarray, v_tgt: np.ndarray) -> np.ndarray:
    v_src = np.asarray(v_src, dtype=float)
    y_src = np.asarray(y_src, dtype=float)
    v_tgt = np.asarray(v_tgt, dtype=float)
    if v_src.size == 0 or y_src.size == 0:
        return np.full_like(v_tgt, np.nan, dtype=float)
    if v_src[0] > v_src[-1]:
        vx = v_src[::-1]
        yy = y_src[::-1]
    else:
        vx = v_src
        yy = y_src
    m = np.isfinite(vx) & np.isfinite(yy)
    if np.count_nonzero(m) < 2:
        return np.full_like(v_tgt, np.nan, dtype=float)
    return np.interp(v_tgt, vx[m], yy[m], left=np.nan, right=np.nan)
...
def _apply_freq_axis_format(ax, labelsize: str = "x-small") -> None:
    try:
...
class ProfileMapGridViewer:
    ...
    def __init__(
        ...
        self.view_mode = "baseline_added" if self.show_baseline else "original"
    ...
    def _print_help(self):
        ...
        print(" w     : Toggle fit windows (BSL_WINF)")
    ...
    def _baseline_curve_for_row(self, i: int, vel_axis: np.ndarray, row_scale: Optional[str] = None) -> Optional[np.ndarray]:
        ...
        nchan = len(self.matrix[i]) if isinstance(self.matrix, list) else int(np.asarray(self.matrix[i]).shape[0])

        if arr.size == nchan:
            baseline_curve = arr.astype(float, copy=True)
        else:
            try:
                baseline_curve = np.polyval(arr.astype(float, copy=False), vel_axis).astype(float, copy=False)
            except Exception:
                return None

        bsl_scale = normalize_tempscal(row.get("BSL_SCALE", row_scale if row_scale is not None else self.tempscals[i]))
        if self.display_scale != bsl_scale:
            ...
            if np.isfinite(eff) and eff > 0:
                ...
                if self.display_scale == "TR*" and bsl_scale == "TA*":
                    baseline_curve = ta_to_tr(base_2d, eff_arr)[0]
                elif self.display_scale == "TA*" and bsl_scale == "TR*":
                    baseline_curve = tr_to_ta(base_2d, eff_arr)[0]

        return np.asarray(baseline_curve, dtype=float)

    def _cell_spectrum(self, idx_list: List[int]) -> Optional[Tuple[pd.Series, np.ndarray, np.ndarray, Optional[np.ndarray], np.ndarray, Optional[np.ndarray]]]:
        if not idx_list:
            return None

        target = self.display_scale

        def _get_scaled_spectrum(i: int) -> np.ndarray:
            y_i = np.asarray(self.matrix[i], dtype=float)
            scale_i = self.tempscals[i]
            if scale_i != target:
                eff_i = self.beameffs[i]
                if target == "TR*" and scale_i == "TA*":
                    y_i = ta_to_tr(y_i, np.array([eff_i]))[0]
                elif target == "TA*" and scale_i == "TR*":
                    y_i = tr_to_ta(y_i, np.array([eff_i]))[0]
            return y_i

        if len(idx_list) == 1:
            i0 = idx_list[0]
            y = _get_scaled_spectrum(i0)
            row = self.st.table.iloc[i0]
            vel_disp, freq_ghz, chan = _row_axes_for_plot(
                self.st.meta, row, len(y), self.rest_hz_meta, self.rest_hz_user, self.rest_hz
            )
            baseline_curve = self._baseline_curve_for_row(i0, vel_disp, row_scale=self.tempscals[i0])
            if self.view_mode == "baseline_added" and baseline_curve is not None:
                y = y + baseline_curve
            return row, y, vel_disp, freq_ghz, chan, baseline_curve

        i_ref = idx_list[0]
        row_ref = self.st.table.iloc[i_ref]
        y_ref = _get_scaled_spectrum(i_ref)
        vel_tgt, freq_tgt, chan_tgt = _row_axes_for_plot(
            self.st.meta, row_ref, len(y_ref), self.rest_hz_meta, self.rest_hz_user, self.rest_hz
        )

        acc = []
        baseline_acc = []
        for i in idx_list:
            row_i = self.st.table.iloc[i]
            y_i = _get_scaled_spectrum(i)
            vel_i, _freq_i, _chan_i = _row_axes_for_plot(
                self.st.meta, row_i, len(y_i), self.rest_hz_meta, self.rest_hz_user, self.rest_hz
            )
            baseline_i = self._baseline_curve_for_row(i, vel_i, row_scale=self.tempscals[i])
            y_use = y_i + baseline_i if (self.view_mode == "baseline_added" and baseline_i is not None) else y_i

            if len(vel_i) == len(vel_tgt) and np.allclose(vel_i, vel_tgt, rtol=0.0, atol=1e-9, equal_nan=True):
                acc.append(np.asarray(y_use, dtype=float))
                if baseline_i is not None:
                    baseline_acc.append(np.asarray(baseline_i, dtype=float))
            else:
                acc.append(_interp_to_velocity_grid(vel_i, y_use, vel_tgt))
                if baseline_i is not None:
                    baseline_acc.append(_interp_to_velocity_grid(vel_i, baseline_i, vel_tgt))

        y = np.nanmean(np.asarray(acc, dtype=float), axis=0)
        baseline_curve = np.nanmean(np.asarray(baseline_acc, dtype=float), axis=0) if baseline_acc else None
        return row_ref, y, vel_tgt, freq_tgt, chan_tgt, baseline_curve
    ...
    def update_plot(self):
        ...
        mode_str = "Raw/Restored" if self.view_mode == "baseline_added" else "Residual"
        title_str = f"Scale: {self.display_scale} | View: {mode_str} | Smooth: {smooth_tag} | RMS Mode: {self.rms_on}"
        ...
        for r in range(self.nrows):
            for c in range(self.ncols):
                ...
                if self.mode == "coadd":
                    ...
                    if got:
                        draw_items.append((got[0], got[1], got[2], got[3], got[4], got[5], "black", 1.0, 0.8, True))
                else:
                    ...
                    for idx in idx_list:
                        ...
                        if got:
                            draw_items.append((got[0], got[1], got[2], got[3], got[4], got[5], line_color, line_alpha, line_lw, False))

                if not draw_items: continue

                for row, y, x_vel_raw, x_freq_raw, x_chan_raw, baseline_curve, color, alpha, lw, is_main in draw_items:
                    if self.xaxis == "vel":
                        x_raw = x_vel_raw
                    elif self.xaxis == "freq" and x_freq_raw is not None:
                        x_raw = x_freq_raw
                    else:
                        x_raw = x_chan_raw

                    x_plot, y_plot = _process_spectrum(x_raw, y, self.smooth_mode, self.smooth_width, self.box_downsample, self.box_policy)
                    ...
                    if self.view_mode == "baseline_added" and baseline_curve is not None:
                        x_bl, y_bl = _process_spectrum(x_raw, baseline_curve, self.smooth_mode, self.smooth_width, self.box_downsample, self.box_policy)
                        ax.plot(x_bl, y_bl, color="red", lw=builtins.max(0.6, lw), alpha=builtins.min(0.85, alpha + 0.15))
                    ...
                    if is_main or (self.mode != "coadd" and row is draw_items[-1][0]):
                        if self.view_mode == "baseline_added" and baseline_curve is None:
                        ...
                        if self.annotate_rms:
                            ...
                            if self.view_mode == "baseline_added" and baseline_curve is not None:
                                y_resid_raw = y - baseline_curve
                            else:
                                y_resid_raw = y
                            data_rms = y_plot if self.rms_on == "display" else y_resid_raw
                            ...
                            if xlim is not None:
                                ...
                                if self.rms_on == "display":
                                    ...
                                    mask_base = (x_raw >= builtins.min(lo, hi)) & (x_raw <= builtins.max(lo, hi))
                                    x_for_win = x_raw
                            else:
                                mask_base = np.ones_like(data_rms, dtype=bool)
                                x_for_win = x_plot if self.rms_on == "display" else x_raw
        ...
        if self.xaxis == "freq":
             for ax in self.axes_flat:
                 if ax.lines or (ax == self.axes[self.nrows-1, 0]):
                     _apply_freq_axis_format(ax, labelsize="x-small")
