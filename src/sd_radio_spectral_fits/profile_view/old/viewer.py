...
class SpectralViewer:
    ...
    def _row_vel_lsrk_and_oriented(self, row: pd.Series, y: np.ndarray):
        ...
        # This velocity is based on the HEADER's Rest Frequency.
        specsys = str(self.st.meta.get("SPECSYS", "")).upper()
        if "LSR" in specsys:
            # Already LSRK, v_corr shouldn't be added again
            v_corr = 0.0
        else:
            v_corr = row.get("v_corr_kms", 0.0)
            if not np.isfinite(v_corr): v_corr = 0.0
            
        vel_native = vlsrk_axis_for_spectrum(self.st.meta, v_corr_kms=float(v_corr), nchan=nchan)
        ...
        if (self.rest_hz_meta is not None and self.rest_hz_meta > 0 and 
            ...
            vel_disp = recalculate_velocity_axis(vel_native, self.rest_hz_meta, self.rest_hz_user)
        ...
        if self.rest_hz is not None and self.rest_hz > 0:
            ...
            try:
                f_obs = _generate_obs_freq_axis(self.st.meta, nchan)
                freq_ghz = f_obs * 1e-9
            except:
                freq_ghz = None
