...
- **support radius**: kernel を 0 とみなす cutoff 半径。一般概念。  
- **truncate**: `gauss` / `gjinc` に対して cutoff 半径を指定する public パラメータ。  
- **convsupport**: `sf` に対する pixel 単位 cutoff 半径。  

本パッケージでは、`sf` の cutoff は通常 `convsupport` で与える。`gjinc` / `gauss` は `truncate` や `support_radius_*` で与える。
...
これは CASA `tsdimaging` docs の formal default (`SF` の `convsupport=3`) に合わせたものである。一方で、ALMA の single-dish imaging notebook では、ALMA data に対して `SF` と `convsupport=6` が推奨例として示されている。したがって、**`3` は formal default、`6` は推奨運用例の一つ**と整理する。
...
\[
\text{explicit pix} > \text{explicit beam} > \text{preset default}
\]
...
\[
cell\_arcsec = beam\_fwhm\_arcsec / 3
\]

へ置換する。現在の clean 実装では、曖昧な手書き `__init__` は使っていない。
...
\[
cut = support\_radius\_pix
\]
\[
isupp = \mathrm{clip}(\lfloor 2cut \rfloor, 4, 8)
\]
\[
ialpha = \mathrm{clip}(\lfloor 2sphparm + 1 \rfloor, 1, 5)
\]
\[
j_{\max} = \min(\lfloor cut \rfloor, 7)
\]
...
\[
	ext{(1) 高空間周波数を十分に表現できず、aliasing が増える}
\]

\[
	ext{(2) signed kernel では ringing や負アーティファクトが出やすくなる}
\]
...
\[
N_{
m used}(p) < n\_min\_avg
\]

の pixel を invalid にする QC 条件である。`GridInput.flag=False` の行や、必要 channel が `NaN` の行は、結果として `N_{
m used}(p)` を減らす。
