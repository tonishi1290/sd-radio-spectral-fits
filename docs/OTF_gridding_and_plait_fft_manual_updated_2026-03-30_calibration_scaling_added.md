...
現在の実装は、OTF 専用の gridding / basketweave だけを切り出して使うより、**calibration を含む前処理から連続した流れ**として考えると分かりやすいです。

特に重要なのは、`grid_otf_family(...)` に渡す入力は「raw dump そのもの」ではなく、通常は

1. `read_scantable(...)` で読む
2. `run_tastar_calibration(...)` で Ta* を作る
3. 必要なら multi-beam の相対スケール補正を行う
4. 必要なら `run_velocity_regrid(...)` で速度軸をそろえる
5. 必要なら `run_baseline_fit(...)` で baseline を引く、または `apply=False` で `BSL_RMS` だけ得る
6. その後に `grid_otf_family(...)` へ渡す

という前処理を経た scantable である、という点です。

### 2.1 single beam の基本流れ

single beam では、流れは比較的単純です。

```text
raw SDFITS / scantable
    -> run_tastar_calibration(...)
    -> (optional) run_velocity_regrid(...)
    -> (optional) run_baseline_fit(...)
    -> grid_otf_family(...)
    -> basketweave_cubes(...)
```
...
- OTF gridding 自体は `BSL_RMS` を inverse-variance 重みに使うので、baseline 後 scantable を入れるとその RMS が重みに効く
- 生スペクトルの baseline はまだ引きたくないが RMS だけ欲しい場合は、`run_baseline_fit(..., apply=False)` を使う
- `cell_arcsec` はまず `beam_fwhm_arcsec / 3` を初期値にする
- X 方向群と Y 方向群は、可能なら同じ velocity grid、同じ map geometry へそろえてから family gridding する

### 2.2 multi-beam の基本流れ

multi-beam では、single beam と違って **beam ごとの相対強度差**をどこで補正するかが重要です。現在の推奨は、**calibration の直後**に相対スケール補正を入れることです。

```text
raw SDFITS / scantable
    -> run_tastar_calibration(...)
    -> apply_relative_scale(...)   # beam間相対補正
    -> (optional) apply_global_scale(...)  # 後日決まった一律係数
    -> (optional) set_beameff(...)         # TR* 変換係数が分かっている場合
    -> (optional) run_velocity_regrid(...)
    -> (optional) run_baseline_fit(...)
    -> grid_otf_family(...)
    -> basketweave_cubes(...)
...
ここで、`apply_relative_scale(...)` と `apply_global_scale(...)` は **data 本体への掛け算**です。  
一方、`set_beameff(...)` は data を変えず、後で `TR*` へ変換するための `BEAMEFF` metadata を入れるだけです。

したがって、beam 間相対補正係数 $g_i$ と後日決まる一律係数 $f$ は

$$
T_i^{\rm aligned}(k)=g_i\,T_i(k)
$$

$$
T_i^{\rm final}(k)=f\,T_i^{\rm aligned}(k)=f\,g_i\,T_i(k)
$$
...
の $\eta$ です。したがって、beam 間相対補正の係数を `BEAMEFF` に流用してはいけません。`BEAMEFF` をその用途に使うと、viewer / coadd / write が自動の `TA* \leftrightarrow TR*` 解釈に使ってしまい、意味が混線します。

### 2.3 OTF 本体の 3 段階

前処理を済ませたあと、OTF 本体は次の 3 段階で考えると分かりやすいです。

#### 2.3.1 family ごとの OTF gridding

X 方向群、Y 方向群のように、**走査方向ごとに分けた入力**をそれぞれ gridding し、family cube を作ります。

```text
preprocessed scantable -> grid_otf_family(...) -> family cube
```

ここでの重みは、各 dump の `BSL_RMS` に基づく inverse-variance weighting が基本です。

#### 2.3.2 family cube の coadd
...
この節では、single beam と multi-beam を分けて書きます。流れをつかむだけなら single beam の方が分かりやすく、そのうえで multi-beam では **calibration 直後に相対スケール補正を入れる**と理解してください。

## 8.1 single beam: calibration を含む最小フロー

まず最も基本的な流れは次です。

1. SDFITS の scantable を読む
2. `run_tastar_calibration(...)` で Ta* を作る
3. 必要なら `run_velocity_regrid(...)` で速度軸をそろえる
4. OTF 用の粗い line-free 窓で `run_baseline_fit(...)` を行う
5. X family / Y family を別々に `grid_otf_family(...)`
6. `basketweave_cubes(...)` で FFT/PLAIT を行う

単一走査方向しか無いなら `basketweave_cubes(...)` は使わず、そのまま family cube を最終生成物としてもよいです。

### single beam の実例

```python
raw_fitsX = "necst_otf_20260321_075602_orion-kl_sdfits.fits"
raw_fitsY = "necst_otf_20260321_090407_orion-kl_sdfits.fits"

sc_rawX = fitsio.read_scantable(raw_fitsX)
sc_rawY = fitsio.read_scantable(raw_fitsY)

sc_cal_12coX = cal.run_tastar_calibration(
    input_data=sc_rawX,
...
sc_cal_12coY = cal.run_tastar_calibration(
    input_data=sc_rawY,
    vlsrk_range_kms=(-35, 60),
    vcorr_chunk_sec=10,
    dtype="float32",
...
sc_vregX = vgr.run_velocity_regrid(
    input_data=sc_cal_12coX,
    vmin_kms=-30.0,
    vmax_kms=55.0,
    dv_kms=0.2,
)

sc_vregY = vgr.run_velocity_regrid(
    input_data=sc_cal_12coY,
    vmin_kms=-30.0,
    vmax_kms=55.0,
    dv_kms=0.2,
)

vwin_baseline = ["-30:0", "20:55"]
npoly = 1

sc_base_12coX = bsl.run_baseline_fit(
    input_data=sc_vregX,
    poly_order=npoly,
    vwin=vwin_baseline,
    overwrite=True,
...
sc_base_12coY = bsl.run_baseline_fit(
    input_data=sc_vregY,
    poly_order=npoly,
    vwin=vwin_baseline,
    overwrite=True,
...
bw_bundle = m3d.basketweave_cubes(
    x_bundle,
    y_bundle,
    plait_noise_mode="family_channel",
    linefree_velocity_windows_kms=["-30:-10", "20:50"],
...
- OTF 前に baseline fitting しておくと、その `BSL_RMS` が OTF gridding の重みに使われる
- 生データの baseline は引きたくないが RMS だけ欲しいときは、`run_baseline_fit(..., apply=False)` を使う
- `cell_arcsec` はまず `beam_fwhm_arcsec / 3` を初期値にする
- basketweave を使わないときは、X と Y を別に作らず `[sc_base_12coX, sc_base_12coY]` を 1 回で gridding してもよい

## 8.2 multi-beam: calibration 直後に相対スケール補正を入れる

multi-beam では、single beam の流れに加えて **beam ごとの相対強度差の補正**を考える必要があります。現在の推奨位置は **calibration の直後**です。
...
が続くからです。相対補正を後回しにすると、`BSL_RMS` や moment0 のような温度スケール依存量が古い scale のまま残りやすくなります。したがって、multi-beam の相対補正は
...
以下は、beam ごとに calibration 後 scantable の data 本体へ相対係数を掛けてから OTF へ進む例です。
...
su.apply_relative_scale(
    sc_cal,
    factors=relative_scale_map,
    key_columns=("FDNUM", "IFNUM", "PLNUM"),
    strict=True,
)
```

ここで `apply_relative_scale(...)` は **data 本体への掛け算**です。  
したがって、各 beam の補正係数を $g_i$ とすると、

$$
T_i^{\rm aligned}(k)=g_i\,T_i(k)
$$

を作ってから、その後の regrid / baseline / OTF gridding へ進みます。

### 後日、一律係数を掛ける例

相対補正を済ませた後で、絶対強度の一律係数 $f$ が決まったときは `apply_global_scale(...)` を使えます。

```python
su.apply_global_scale(sc_cal, factor=1.035)
```

このとき data は

$$
T_i^{\rm final}(k)=f\,T_i^{\rm aligned}(k)
$$
...
後で決まった係数が generic な一律補正ではなく、物理的な `TR*` 変換係数 $\eta$ であるなら、`apply_global_scale(...)` ではなく `set_beameff(...)` を使って `BEAMEFF` を設定します。
...
- `set_beameff(...)` は data を変えない
- `BEAMEFF` は
  $$
  T_R^* = \frac{T_A^*}{\eta}
  $$
  の $\eta$ である
- したがって、beam 間相対補正係数を `BEAMEFF` に流用してはいけない

## 8.3 preprocessed scantable から family cube を作って PLAIT

前節までの前処理を済ませたあと、OTF 本体の最小経路は次です。
...
## 8.4 family cube を FITS に保存してから basketweave
...
## 8.5 既存 cube を coadd してから PLAIT
...
## 8.7 tile ごとに BW してから mosaic する
...
- 通常は **calibration 済み**、必要なら **multi-beam 相対補正済み** の入力を受ける
- velocity axis を揃える
...
- raw そのものではなく、通常は `run_tastar_calibration(...)` 後の scantable を渡す
- multi-beam の場合は、beam 間相対補正が必要なら `apply_relative_scale(...)` を calibration 直後に適用してから渡す
