...
本書の主題は SDFITS の書式・writer・reader 契約です。可視化モジュール `sd_radio_spectral_fits.profile_view` の詳細は、最新版の `profile_view_manual_ja_v2.md` を正本として参照してください。この SDFITS 説明書では profile_view の内部仕様までは重複して展開しません。
...
`post_baseline_mode='inherit_all'` を使うと、前段 baseline 設定を継承できます。ただし継承元が必要なので、`baseline_vwin` を与えていないと使えません。なおこのモードは窓・次数・反復設定を一括継承するため、`post_baseline_vwin='inherit'` などの個別 `inherit` 指定は不要です。これらの個別引数は、継承後に一部だけを明示値で上書きしたい場合のために残っています。
...
$$
T_{cal} = T_{hot} e^{\tau X} - T_{atm} \left(e^{\tau X} - 1\right) - T_{bg}
$$
...
#### 9.7 calibration 直後に相対スケール補正を入れる

multi-beam で beam 間相対係数がある場合、`run_tastar_calibration()` の直後に `apply_relative_scale()` を掛け、その後に baseline / RMS / moment / coadd へ進むのが安全です。

相対係数 $g_i$ と generic な一律係数 $f$ は、どちらも data 本体へ**掛ける**係数です。
...
の $\eta$ です。したがって、generic な一律係数が必要なら `apply_global_scale()` を使い、後で決まった量が物理 `TR*` 変換の $\eta$ である場合は `set_beameff()` を設定して後段の `TR*` 変換へ渡す方が意味論として安全です。`set_beameff()` 自体は data 本体を掛け直しません。

#### 9.8 `recalibrate_tastar()`
...
#### 10.3 `set_beameff()` / `apply_relative_scale()` / `apply_global_scale()`

`set_beameff()` は `Scantable` の `BEAMEFF` を row 単位または一括で設定します。これは metadata setter であり、data 配列そのものは変えません。

一方、beam 間相対スケール合わせや後日の一律 factor 適用には、`apply_relative_scale()` / `apply_global_scale()` を使います。これらは data 本体へ直接係数を掛け、`TEMPSCAL` は保持し、累積 factor を `INTSCALE` に記録します。既定では `BSL_RMS`, `BSL_COEF`, `BSL_SCALE`, `MOMENT0`, `MOM0`, `INTEGRATED` を無効化します。したがって、**calibration 直後に適用してから baseline / moment / coadd を行う**のが推奨です。
...
前段 baseline を group 内各スペクトルに掛ける経路と、coadd 後に代表スペクトルへ掛ける経路は別です。後者で作られる `BSL_*` は `BSL_STAGE='post_coadd'` になります。なお `post_baseline_mode='inherit_all'` を使うとき、上の個別 `post_baseline_*` 引数へさらに `inherit` を明示指定する必要はありません。個別引数は一部を明示値で上書きしたいときだけ使います。
