...
## 6.4 相対スケール補正と `BEAMEFF` は分ける

beam 間の相対スケール合わせは、物理 `BEAMEFF` とは別物として扱う方が安全です。最新版では

- `set_beameff()` は物理 `BEAMEFF` metadata を設定するだけ
- `apply_relative_scale()` は row ごとの相対係数 $g_i$ を data へ直接掛ける
- `apply_global_scale()` は一律係数 $f$ を data へ直接掛ける
...
これら 2 つの scale 適用関数は `TEMPSCAL` を変えず、累積 factor を `INTSCALE` に記録します。既定では `BSL_RMS`, `BSL_COEF`, `BSL_SCALE`, `MOMENT0`, `MOM0`, `INTEGRATED` を無効化するので、**calibration 直後に適用してから baseline / moment / coadd へ進む**のが推奨です。
...
を post-coadd 側へ継承するモードです。`baseline_vwin` が指定されていないと使えません。また、このモードを使うなら `post_baseline_vwin="inherit"` などの個別 `"inherit"` 指定は不要です。個別に変えたい項目があるときだけ、その項目へ明示値を与えます。
...
### `set_beameff()` / `apply_relative_scale()` / `apply_global_scale()`

#### `set_beameff()`
物理 `BEAMEFF` を row-wise に設定する metadata 更新です。data 配列は変えません。scalar / 配列 / mapping を受け、`key_columns=("FDNUM", "IFNUM", "PLNUM")` で multi-beam 指定もできます。

#### `apply_relative_scale()`
beam ごとの相対係数を data 本体へ直接掛けます。`TEMPSCAL` は保持され、累積 factor は `INTSCALE` に記録されます。scale 依存派生列は既定で無効化されます。

#### `apply_global_scale()`
後で決まった一律 factor を全 row または選択 row へ掛けます。使い方は `apply_relative_scale()` の単純版です。

#### 推奨順序
`run_tastar_calibration()` 直後に `apply_relative_scale()` を掛け、必要ならさらに `apply_global_scale()` を掛けてから baseline / RMS / moment / coadd へ進むのが最も安全です。

#### `TEMPSCAL` / `BEAMEFF` との関係
`TR*` で扱うなら物理 `BEAMEFF` が必要です。相対スケール合わせ用途で `BEAMEFF` を流用しない方が混乱が少なくなります。
...
`profile_view` は package 全体の quick-look / 可視化を担当する層です。最新版の詳細仕様は `profile_view_manual_ja_v2.md` を正本として参照してください。本書では package manual として必要な位置づけだけを残し、重複説明は避けます。

### 主要 API

最新版の profile_view 説明書では、上位 API として少なくとも次が整理されています。
...
- 入力は `Scantable` またはファイルパス
- row ごとの WCS, `RESTFRQ/RESTFREQ`, `SPECSYS/SSYSOBS`, `VELOSYS/VFRAME`, `TEMPSCAL`, `BEAMEFF` を尊重して表示する
- 解析結果の quick-look と、人間による異常確認の最前線を担う
...
pv.view_spectra(sc, xaxis="vel", xrange=(-20, 40))
```
...
- viewer のキーボード操作
- `rest_freq` 上書きと `BSL_WINF` の相互作用
- `montage` と `grid` のページ構成
- `grid` の格子割り当て、`projection`, `grid_tol`, `grid_anchor_offsets`
- PDF 出力、固定紙面サイズ、余白指定

は `profile_view_manual_ja_v2.md` を参照してください。
...
6. 必要なら `post_baseline_mode='inherit_all'` で仕上げる  
...
ここでシグネチャ中に `post_baseline_vwin='inherit'` などが見えますが、これは内部既定値です。利用者が `post_baseline_mode='inherit_all'` を使うときに、これらを明示的に `"inherit"` と書く必要はありません。個別に上書きしたい項目がある場合だけ、その項目へ明示値を与えます。
...
これは便利ですが、前段の `baseline_vwin` が妥当であることを前提にします。前段窓が広すぎたり線を食っていると、その設定がそのまま post-coadd にも伝播します。したがって、まず pre-coadd の `BSL_RMS` と `BSL_COEF` を見て、窓設定が妥当か確認するのがよいです。なお `inherit_all` は窓・次数・反復設定を一括継承するので、`post_baseline_vwin="inherit"` などの個別 `"inherit"` 指定を併記する必要はありません。シグネチャに既定値として見えていても、それを利用者が毎回書く必要はない、という意味です。
