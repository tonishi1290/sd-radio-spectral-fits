...
- `sunscan_extract_multibeam` では、DB 名解決と天体位置計算のために次の 3 つの値を使います
  - `--telescope`（例: `OMU1P85M`）
  - `--tel-loaddata`（例: `OMU1p85m`）
  - `--planet`（例: `sun`）
- 現在の既定値は `OMU1P85M / OMU1p85m / sun` です。OMU 1.85 m の太陽スキャンでは通常この既定値のままで構いませんが、**望遠鏡名や対象天体が異なる場合はここを必ず合わせてください。**
...
ここで指定する `--telescope`, `--tel-loaddata`, `--planet` は、single-beam の `sunscan_singlebeam` と同じ意味です。multi-beam でも、各 stream の解析は内部的には single-beam 相当解析を繰り返しているため、**この 3 つの値が DB 名解決と天体位置計算の根本パラメータ**になります。
