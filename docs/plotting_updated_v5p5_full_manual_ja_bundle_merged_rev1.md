...
### 3.4 OTF bundle の 2D extension を直接見る

```python
pl.quicklook(bundle, ext='MOMENT0')
```

ここで `bundle` は `OTFBundle` です。`MOMENT0`, `RMS`, `WEIGHT`, `HIT`, `MOSAIC_GAIN` など、**2D ext を明示したとき**は 2D map としてそのまま表示できます。

### 3.5 OTF bundle の 3D science cube から moment0 を作って見る

```python
pl.quicklook(bundle, mode='moment0', vel_range=(20, 40))
```

`OTFBundle` に対して `ext=None`（または `ext=0`, `ext='PRIMARY'`, `ext='DATA'`）を使うと、science cube `bundle.data` が選ばれます。したがって、`mode='moment0'` や `mode='channel_slice'` などの 3D reduction がそのまま使えます。

### 3.6 baseline viewer bundle をそのまま表示する

```python
from sd_radio_spectral_fits.map_3d import make_baseline_viewer_bundle

viewer = make_baseline_viewer_bundle(bundle, mode='signal')
pl.quicklook(viewer, mode='moment0', vel_range=(20, 40))
```

viewer bundle は plotting 側では特別扱いしません。**通常の `OTFBundle` と同じ source**として扱います。違いは `data` に、`SIGNAL_MASK_USED` または `LINEFREE_USED` で切った 3D cube が入っている点だけです。

### 3.7 color を bundle、contour を FITS にする
...
pl.plot_map(
    ...
    ext='MOMENT0',
    contours=[
        {'source': '13co_cube.fits', 'mode': 'moment0', 'vel_range': (20, 40), 'colors': 'cyan'}
    ],
)
```

### 3.8 color を FITS、contour を bundle にする

```python
pl.plot_map(
    'optical_or_radio_2d.fits',
    contours=[
        {'source': bundle, 'ext': 'MOMENT0', 'colors': 'white', 'levels': 'auto'}
    ],
)
```

color と contour は **独立に source を解決**します。したがって `path / HDU / HDUList / OTFBundle / Map2D` が混在していても構いません。
...
- `OTFBundle`（ただし **2D として読むには 2D ext を選ぶ必要がある**）

`OTFBundle` を 2D source として使う典型例:
...
`OTFBundle` を 3D source として使う典型例:
...
- `ext='DATA'`
- `ext='VARIANCE'`
- `ext='VALID_MASK'`（3D valid mask を持つ場合）
- baseline viewer bundle の `data`

### 4.3 `ext=None` の意味

- FITS file や `HDUList` を渡し、`ext=None` にすると、**最初に data を持つ HDU** が選ばれます。
- `OTFBundle` を渡し、`ext=None` にすると、**primary science cube (`bundle.data`)** が選ばれます。

したがって、`plot_map(bundle)` のように **2D が必要な場面で ext を省略すると 3D cube が選ばれる**ため、そのままでは表示できません。`plot_map(bundle, ext='MOMENT0')` のように 2D ext を明示してください。

### 4.4 OTF bundle の ext の考え方

`OTFBundle` では、FITS のように各 ext が個別の header を持つのではなく、基本的に
...
という in-memory 構造で保持されています。

plotting 側では image-like なものだけを使います。扱いの要点は次のとおりです。

#### `OTFBundle` で解釈される主な ext 名

- `None`, `0`, `'PRIMARY'`, `'DATA'` → `bundle.data`
- `'VARIANCE'` → `bundle.variance`
- `'VALID_MASK'` → `bundle.valid_mask`
- `'SUPPORT_MASK'` → `bundle.support_mask`
- その他の文字列 → `bundle.image_ext[...]` を **大文字小文字をあまり気にせず探索**

#### integer ext の注意

`OTFBundle` は FITS の `HDUList` ではないので、**一般の integer ext 指定には対応しません**。現在の実装で許しているのは **`ext=0` を primary/data と解釈する場合だけ**です。

### 4.5 OTF bundle における 2D / 3D / 1D の整理

`OTFBundle` の ext は、配列の shape と意味を分けて考える必要があります。
...
ここは重要です。**現行 baseline/bundle 仕様では `LINEFREE*` と `SIGNAL_MASK_USED` は global channel mask であり、各 spatial pixel ごとの 3D mask ではありません。** 典型的な shape は `(nchan,)` です。

したがって、これらは `plot_map(..., ext='LINEFREE_USED')` のように 2D map として直接描く対象ではありません。`make_2d_map(..., mask_mode='linefree_complement')`、provisional/final moment、または `make_baseline_viewer_bundle(...)` の材料として使います。

補足として、plotting の内部 helper は互換性のために

- `(nchan,)`
- `(nchan,1)` や `(1,nchan)`
- `(nchan,1,1)`
- `(nchan,ny,nx)` から channel 軸を落として解釈できるケース

も一部受理します。ただし、**現行の baseline が書き出す正規形は global 1D mask** と理解してください。

### 4.6 2D passthrough の挙動
...
`OTFBundle` 自体は `plotting.py` の中で定義されている dataclass ではなく、`map_3d.otf_bundle` 側の in-memory bundle です。plotting 側ではこれを **FITS path / HDU / HDUList と並ぶ source** として受け取ります。主要フィールドは次のとおりです。

- `data`: science cube, shape = `(nchan, ny, nx)`
- `header`: WCS と科学メタデータを持つ FITS header
- `variance`: variance cube
- `valid_mask`: `(nchan, ny, nx)` または `(ny, nx)`
- `support_mask`: `(ny, nx)`
- `unit`: data unit
- `family_label`: family label
- `image_ext`: 追加の 2D/3D image ext
- `table_ext`: table ext
- `meta`: 実行時 metadata

plotting が実際に使うのは主に `data`, `header`, `variance`, `valid_mask`, `support_mask`, `image_ext` です。`table_ext` は現時点の plotting では直接は描画しません。

### 5.5 baseline viewer bundle

baseline 系の `make_baseline_viewer_bundle(bundle, mode='signal'|'linefree')` は、**新しい型を返すのではなく、通常の `OTFBundle` を返します。** したがって plotting 側では viewer 専用 API を増やしていません。

viewer bundle の意味は

- `mode='signal'` → `SIGNAL_MASK_USED` の channel だけを `data` に残す
- `mode='linefree'` → `LINEFREE_USED` の channel だけを `data` に残す

です。`meta` には

- `baseline_viewer_mode`
- `baseline_viewer_fill_value`
- `baseline_viewer_mask_key`

などが残りますが、plotting の使い方自体は通常の `OTFBundle` と同じです。
...
- `OTFBundle`（2D ext を明示した場合）

`OTFBundle` を 2D source として使うときは、通常 `ext='MOMENT0'` のように **2D ext を選びます**。`ext=None` だと primary/data の 3D cube が選ばれるため、2D resolver ではエラーになります。
...
- `OTFBundle` の 2D ext は個別 header を持たないため、**2D 用 header は `bundle.header` から celestial WCS を落として再構成**します。
- unit も ext ごとの意味から補います。たとえば `MOMENT0` は概ね `data_unit * spectral_unit`、`MOMENT1` は速度単位、mask や weight 系は unit なしです。
...
- 3D 以外を渡すとエラーです。
- `OTFBundle` では `ext=None`, `ext=0`, `ext='PRIMARY'`, `ext='DATA'` が science cube を意味します。
- `OTFBundle` の viewer bundle も通常の bundle と同じなので、そのまま cube source として使えます。
...
#### `source`
対象 source。FITS path / HDU / HDUList / `(header, data)` / `Map2D` / `SpectralCube` / `OTFBundle` など、`make_2d_map()` が理解できるものを渡せます。
...
#### `source`
2D/3D source。FITS path, HDU, HDUList, `(header, data)`, `SpectralCube`, `OTFBundle` などを渡せます。

#### `ext`
対象 HDU または extension selector。FITS 系では HDU name/index、`OTFBundle` では `PRIMARY/DATA/VARIANCE/VALID_MASK/SUPPORT_MASK` または `image_ext` の名前を与えます。

`OTFBundle` の注意:

- `ext=None` は science cube `bundle.data` を意味します
- 2D ext を選べば passthrough になります
- 3D ext を選べばその 3D 配列に対して reduction が行われます
- `LINEFREE*` や `SIGNAL_MASK_USED` は現行仕様では 1D spectral mask なので、2D map ext としては扱いません
...
これらは FITS path/HDUList だけでなく、`OTFBundle.image_ext` からも探索されます。したがって、baseline 後 bundle に入った `LINEFREE`, `LINECAND3D`, `BASESUP3D`, `MASK3D` をそのまま参照できます。
...
- `OTFBundle` の 1D spectral mask (`LINEFREE*`, `SIGNAL_MASK_USED`) は、current plotting では 2D map として直接は描きません。これらは `mask_mode='linefree_complement'`、provisional/final moment、または viewer bundle の入力として使います。
...
`source` は FITS path/HDUList だけでなく `OTFBundle` でも構いません。bundle の場合は、これらの associated ext を `bundle.image_ext` から探します。
...
`source` は `OTFBundle` でも構いません。bundle の場合は `MASK3D` を `image_ext` から参照します。
...
ここでの `source` は FITS path / HDU / HDUList / `(header, data)` / `Map2D` / `OTFBundle` などを混在させて構いません。`add_contours()` 自体には quicklook 固有の継承ロジックはありませんが、`quicklook(contours=...)` から呼ばれる場合には、**内部で quicklook の base source / mode / vel_range などを引き継いだ contour layer** が組み立てられます。

したがって、ユーザーから見た使い方としては

- `plot_map(base_map, contours=[{'source': other_source, ...}])`
- `quicklook(source, contours=[{'levels': 'auto'}])`
...
- ここでの `source` は、FITS path / HDU / HDUList / `Map2D` / `(header, data)` / `OTFBundle` を含みます。
...
- **color と contour は source を独立に解決**します。したがって `plot_map(bundle, ext='MOMENT0', contours=[{'source': 'other.fits', ...}])` も、その逆も自然に書けます。
...
- `OTFBundle` を contour source に使う場合も、2D contour には `ext='MOMENT0'` など 2D ext を選ぶか、`mode='moment0'` 等で 3D から 2D を作る必要があります。
...
`source` に `OTFBundle` を渡す場合の典型例は `plot_map(bundle, ext='MOMENT0')` です。`plot_map(bundle)` のように ext を省略すると primary/data の 3D cube が選ばれるため、そのままでは 2D plot はできません。
...
ここでも `source` は FITS path / HDU / HDUList / `(header, data)` / `Map2D` / `OTFBundle` などを混在させて構いません。`plot_map()` 自体には quicklook のような自動継承はありませんが、color と contour は独立に source を解決するため、`plot_map(bundle, ext='MOMENT0', contours=[{'source': 'other.fits', ...}])` も、その逆も自然に書けます。
...
### OTF bundle を `plot_map()` に渡すときの注意

- **2D ext を明示する**: `plot_map(bundle, ext='MOMENT0')`
- **viewer bundle も同じ**: `plot_map(viewer, ext='MOMENT0')` あるいは `quicklook(viewer, mode='moment0', ...)`
- **1D mask は直接描かない**: `LINEFREE*`, `SIGNAL_MASK_USED` は `plot_map(..., ext=...)` の対象ではありません
- contour は独立 source なので、color=bundle / contour=fits も自然に使えます
...
ここでいう bare source には FITS path, HDU, HDUList, `(header, data)`, `OTFBundle` などが含まれます。ただし `OTFBundle` を bare source のまま渡すと primary/data の 3D cube が選ばれるため、通常は

```python
{'source': bundle, 'ext': 'MOMENT0'}
```

のように **2D ext を持つ map specification dict** として与える方が安全です。
...
#### `source`
描画対象 source。FITS path / HDU / HDUList / `(header, data)` / `SpectralCube` / `OTFBundle` などを受けられます。
...
ここでも `source` は FITS path / HDU / HDUList / `(header, data)` / `Map2D` / `OTFBundle` などを混在させて構いません。たとえば `quicklook(bundle, ext='MOMENT0', contours=[{'source': 'other.fits', ...}])` も、その逆も可能です。
...
### OTF bundle を `quicklook()` に渡すときの考え方

- `quicklook(bundle, ext='MOMENT0')` → 2D ext をそのまま表示
- `quicklook(bundle, mode='moment0', vel_range=(...))` → `bundle.data` から 2D map を作る
- `quicklook(viewer_bundle, mode='moment0', ...)` → viewer cube を通常の 3D cube と同様に扱う
- `quicklook(bundle, ext='LINEFREE_USED')` のような 1D spectral mask の直接表示は想定していません
...
layer dict の `source` は、基本的に次を同列に扱います。

- FITS path (`str` / `Path`)
- `fits.HDUList`
- 2D/3D HDU
- `(header, data)`
- `Map2D` / `RGBMap` / `SpectralCube`
- `OTFBundle`

重要なのは、**base layer と overlay layer がそれぞれ独立に source を解決する**ことです。したがって、たとえば

- base=`{'kind': 'image', 'source': bundle, 'ext': 'MOMENT0'}`
- overlay=`{'kind': 'contour', 'source': '13co_cube.fits', 'mode': 'moment0', ...}`
...
ここでの `source` も `OTFBundle` を含みます。`kind='image'` で bundle を使う場合は、通常

- `ext='MOMENT0'` など 2D ext を選ぶ
- あるいは `mode='moment0'` などを指定して primary/data から 2D を作る
...
contour overlay でも `source` は `OTFBundle` を含みます。2D contour を bundle から作る方法は base layer と同じです。
...
- `OTFBundle` の各 image ext は、FITS のように ext ごとの独立 header を持っているわけではありません。plotting では **bundle.header から必要な 2D/3D header を再構成**します。
- そのため、bundle ext ごとの独立な WCS や独立な BUNIT を細かく持ち回る設計には、現時点ではなっていません。2D/3D の ext は **bundle と同じ空間 WCS 上にある**という前提です。
- `LINEFREE`, `LINEFREE_USED`, `SIGNAL_MASK_USED` は current baseline/bundle 仕様では **global 1D spectral mask** です。各 spatial pixel ごとの 3D mask ではありません。
- plotting の helper には一部互換的な shape 受理がありますが、意味論としては上の 1D global mask を前提に読むのが安全です。
...
## 12A.6 OTF bundle は plotting の正式入力 source になった

今回の改訂で、`OTFBundle` は `plot_map()` / `quicklook()` / `plot_scene()` / `add_contours()` / `make_2d_map()` / `resolve_*()` から **正式に扱える source** になりました。

考え方は単純です。

- 2D が欲しいときは `ext='MOMENT0'` のように 2D ext を明示する
- 3D から 2D を作りたいときは `mode='moment0'`, `mode='channel_slice'` などを使う
- contour 側も同じ source 体系を使う

## 12A.7 baseline viewer bundle は特別な型ではない

`make_baseline_viewer_bundle(...)` が返すものは viewer 専用オブジェクトではなく、通常の `OTFBundle` です。したがって、plotting 側の理解としては

- source は `OTFBundle`
- 違いは `data` に入っている 3D cube の中身だけ

です。これにより、viewer だけ別 API を増やさずにすみ、利用者から見ても理解しやすくなっています。
...
### OTF bundle の 2D ext をそのまま表示

```python
pl.quicklook(bundle, ext='MOMENT0')
```

### baseline viewer bundle を表示
